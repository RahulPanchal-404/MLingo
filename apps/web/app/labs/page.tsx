...
const labs = [
  ...
  { href: "/labs/neural-network", tag: "Lab 04", title: "Neural Network & Backpropagation", description: "Watch a 2-layer network learn nonlinear decision boundaries using exact analytical backpropagation.", observation: "You will observe hidden activations, layer gradients, weight norms, and the curving probability surface." },
];

export default function LabsPage() {
  return <SectionPage title="ML Labs" description="Interactive algorithm laboratories built around one idea: understand training by reviewing it frame by frame."><div className="challenge-list">{labs.map((lab) => <Link className="challenge-card" href={lab.href} key={lab.href}><p className="eyebrow">{lab.tag}</p><h2>{lab.title}</h2><p>{lab.description}</p><div className="mt-3 text-sm text-slate-600"><strong>Inspect:</strong> {lab.observation}</div><div className="mt-4 text-sm font-medium text-teal-700">Open laboratory</div></Link>)}</div></SectionPage>;
}
