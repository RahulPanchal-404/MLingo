...
export function ExperimentWorkspace() {
      ...
      const selectAlgorithm = (algorithm: TrainingRunRequest["algorithm"]) => setRequest((current) => algorithm === "logistic_regression" ? { algorithm, dataset: { samples: 64, noise: 0.1, seed: 0 }, training: { ...current.training, learning_rate: 0.2 } } : { algorithm, dataset: { samples: 32, slope: 2, intercept: 1, noise: 0, seed: 0 }, training: { ...current.training, learning_rate: 0.1 } });
      const runExperiment = async (event: React.FormEvent<HTMLFormElement>) => {
            ...
            try {
                  ...
                  recordRunConcepts(request.algorithm === "logistic_regression" ? ["Logistic Regression", "Sigmoid", "Probability", "Binary Cross-Entropy", "Decision Boundary"] : ["Gradient Descent", "Loss", "Learning Rate"]);
      ...
      return <div className="experiment-workspace">
            ...
            <form className="experiment-form" onSubmit={runExperiment}><div className="experiment-form-heading"><div><p className="eyebrow">Algorithm-aware workspace</p><h2>Training configuration</h2></div><span>{loading ? "Running" : "Ready"}</span></div><label className="number-control"><span>Algorithm</span><select aria-label="Algorithm" onChange={(event) => selectAlgorithm(event.target.value as TrainingRunRequest["algorithm"])} value={request.algorithm}><option value="linear_regression">Linear Regression</option><option value="logistic_regression">Logistic Regression</option></select></label><NumberControl label="Learning rate" max="2" min="0.001" step="0.001" value={request.training.learning_rate} onChange={(value) => updateTraining("learning_rate", value)} /><NumberControl label="Epochs" max="300" min="1" step="1" value={request.training.epochs} onChange={(value) => updateTraining("epochs", value)} /><NumberControl label="Samples" max="200" min="2" step="2" value={request.dataset.samples} onChange={(value) => updateDataset("samples", value)} /><NumberControl label="Noise" max="2" min="0" step="0.05" value={request.dataset.noise} onChange={(value) => updateDataset("noise", value)} /><button className="primary-button" disabled={loading} type="submit">{loading ? "Running experiment..." : "Run experiment"}</button></form>
            {error && <section className="lab-alert" role="alert"><strong>Experiment failed.</strong><span>{error}</span></section>}
            {run && <section className="recorded-experiment"><div className="recorded-heading"><div><p className="eyebrow">Recorded experiment</p><h2>{run.algorithm}</h2></div><span>{run.total_steps} steps</span></div><dl className="experiment-summary"><div><dt>Final loss</dt><dd>{formatNumber(run.history.at(-1)?.loss)}</dd></div><div><dt>Samples</dt><dd>{run.dataset.samples}</dd></div><div><dt>Learning rate</dt><dd>{run.training.learning_rate}</dd></div></dl><div className="visual-grid">{run.algorithm.startsWith("logistic") ? <ClassificationPlot points={run.dataset_points} state={state} /> : <RegressionPlot points={run.dataset_points} state={state} />}<LossChart currentStep={timeline.currentStep} history={run.history} label={run.algorithm.startsWith("logistic") ? "Binary Cross-Entropy" : "MSE"} /></div>{run.algorithm.startsWith("logistic") && <section className="learning-modes-grid"><LogisticMathMode learningRate={run.training.learning_rate} state={state} /><LogisticCodeMode learningRate={run.training.learning_rate} state={state} /></section>}<TrainingSignals events={diagnostics} onSelect={timeline.jumpToStep} /><TimelineControls currentStep={timeline.currentStep} isPlaying={timeline.isPlaying} markers={[]} onAddMarker={() => undefined} onBackward={timeline.stepBackward} onForward={timeline.stepForward} onJump={timeline.jumpToStep} onPlayToggle={timeline.togglePlay} onRemoveMarker={() => undefined} onReset={timeline.reset} onSpeed={timeline.setPlaybackSpeed} playbackSpeed={timeline.playbackSpeed} reducedMotion={timeline.reducedMotion} totalSteps={timeline.totalSteps} /></section>}
      </div>;
}
