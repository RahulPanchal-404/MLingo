...
export function ModelXRayPreview() {
  ...
  return (
    <section className="space-y-6" aria-labelledby="xray-preview-heading">
      ...
      {/* Model X-Ray Container (Dual Theme Technical Aesthetic) */}
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 sm:p-8 shadow-xs dark:shadow-2xl text-slate-900 dark:text-slate-100 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-mono text-xs font-bold uppercase tracking-wider text-teal-800 dark:text-teal-300">
              MODEL X-RAY // INTERNAL STATE INSPECTOR
            </span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-slate-500 dark:text-slate-400 mr-1 hidden sm:inline">
              SELECT FRAME:
            </span>
            {INSPECTION_STEPS.map((s) => (
              <button
                ...
                className={`rounded-lg px-3 py-1 font-mono text-xs font-semibold transition-all cursor-pointer ${
                  activeStep === s
                    ? "bg-teal-700 text-white shadow-xs font-bold dark:bg-teal-500 dark:text-slate-950 dark:shadow-md dark:shadow-teal-500/30"
                    : "bg-slate-100 border border-slate-200 text-slate-700 hover:bg-slate-200 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
                }`}
              >
                Step {s}
              </button>
        ...
        {/* Big State Badge */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-slate-50 dark:bg-slate-900/60 rounded-xl p-4 border border-slate-200 dark:border-slate-800">
          <div>
            <span className="text-[10px] font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">
              ACTIVE FRAME
            </span>
            <h3 className="text-xl font-mono font-bold text-slate-900 dark:text-white">
              STEP {currentFrame.step} — {currentFrame.phaseLabel}
            </h3>
          </div>
          <div className="flex items-center gap-4 text-xs font-mono">
            <div>
              <span className="text-slate-500 dark:text-slate-400 text-[10px] block">FRAME LOSS</span>
              <span className="text-amber-600 dark:text-amber-400 font-bold text-sm">
                {currentFrame.loss.toFixed(3)}
              </span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 text-[10px] block">Δ LOSS VS PREV</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/70 p-4 space-y-3">
            <div className="flex items-center justify-between text-xs font-mono text-teal-800 dark:text-teal-400 border-b border-slate-200 dark:border-slate-800 pb-2">
              <span className="font-bold">1. PARAMETERS</span>
              <span className="text-[10px] text-slate-500 dark:text-slate-400">y = wx + b</span>
            </div>

            <div className="space-y-3 font-mono">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block">WEIGHT (w)</span>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block">BIAS (b)</span>
                  <span className="text-base font-bold text-slate-900 dark:text-white">
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed border-t border-slate-200 dark:border-slate-800/60 pt-2">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/70 p-4 space-y-3">
            <div className="flex items-center justify-between text-xs font-mono text-teal-800 dark:text-teal-400 border-b border-slate-200 dark:border-slate-800 pb-2">
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400">Calculus Chain Rule</span>
            </div>

            <div className="space-y-3 font-mono">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block">WEIGHT GRADIENT</span>
                  <span className="text-base font-bold text-amber-600 dark:text-amber-300">
                    {currentFrame.weightGradient.toFixed(3)}
                  </span>
                </div>
                <span className="text-[11px] text-slate-500 dark:text-slate-400">∂L/∂w</span>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block">BIAS GRADIENT</span>
                  <span className="text-base font-bold text-amber-600 dark:text-amber-300">
                    {currentFrame.biasGradient.toFixed(3)}
                  </span>
                </div>
                <span className="text-[11px] text-slate-500 dark:text-slate-400">∂L/∂b</span>
              </div>

              <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-800/60 pt-1.5">
                <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase">Gradient Norm ||∇||</span>
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/70 p-4 space-y-3">
            <div className="flex items-center justify-between text-xs font-mono text-teal-800 dark:text-teal-400 border-b border-slate-200 dark:border-slate-800 pb-2">
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400">Test Feature x = 1.0</span>
            </div>

            <div className="space-y-3 font-mono">
              <div>
                <span className="text-[10px] text-slate-500 dark:text-slate-400 block">PREDICTED VALUE ŷ</span>
              ...
              <div>
                <span className="text-[10px] text-slate-500 dark:text-slate-400 block">GROUND TRUTH VALUE y</span>
                <span className="text-sm font-bold text-slate-600 dark:text-slate-400">3.05 (Error = {(Math.abs(3.05 - currentFrame.predictionAtOne)).toFixed(2)})</span>
              </div>
            </div>
        ...
        <div className="rounded-xl border border-teal-200 dark:border-teal-500/30 bg-teal-50/80 dark:bg-teal-950/40 p-4 font-mono text-xs text-teal-900 dark:text-teal-200 flex items-start gap-3">
          <span className="text-base text-teal-600 dark:text-teal-400">🔬</span>
          <div>
            <strong className="text-teal-950 dark:text-white block mb-0.5">
